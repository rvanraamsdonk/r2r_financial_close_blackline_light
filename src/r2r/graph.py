...
def build_graph(*, data_repo, policy):
    ...
    graph.add_edge("ingest","reconcile")
    graph.add_edge("ingest","match")
    graph.add_edge("ingest","variance")
    graph.add_edge("ingest","intercompany")
    graph.add_edge("reconcile","journals")
    graph.add_edge("match","journals")
